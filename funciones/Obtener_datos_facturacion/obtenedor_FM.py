...
from Constantes import constantes_datos_FM
...
class Obtenedor_FM(obtenedor_datos_facturacion):
    ...
    def obtener_datos_facturacion(self, data_source, datos_factura_manager, datos_usuario):
        ...
        for i in range(len(datos_factura)):
            self.calcular_datos_items_tributos(datos_factura[i][constantes_datos_FM.pos_items], datos_factura[i][constantes_datos_FM.pos_tributos], datos_factura[i], datos_usuario)
