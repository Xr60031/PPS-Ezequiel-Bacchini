...
class Verificador_CUIT():
    def __init__(self):
        self

    def obtener_digitos_cuit(self, CUIT): 
        primeros_diez_digitos = []
        for i in range(10):
            primeros_diez_digitos.append(CUIT[i])
        return primeros_diez_digitos

    def calcular_peso(self, primeros_diez_digitos):
    ...
    def verificar_cuit(self, CUIT):
        ...
        if len(str_CUIT) == 11:
            ...
            if PDD in ['20', '23', '24', '27', '30', '33', '34']:
                CUIT_raw = self.obtener_digitos_cuit(str_CUIT[:10])
                Peso_CUIT = self.calcular_peso(CUIT_raw)
