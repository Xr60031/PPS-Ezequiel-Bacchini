...
def guardar_configuracion(file_name, config):
  
  excel_dataframe=load_workbook(file_name)
...
def guardar_configuracion_inicial(file_name, config, output_file):
  excel_dataframe=load_workbook(file_name, data_only=False)
  datos_hoja=excel_dataframe["Datos"]
  datos_hoja["A2"]=config[0]
  datos_hoja["B2"]=config[1]
  datos_hoja["C2"]=config[2]
  ...
  datos_hoja["K2"]=config[10]
  datos_hoja["L2"]=config[11]
  excel_dataframe.save(output_file)